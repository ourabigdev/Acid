...
ButtonInputAxis::ButtonInputAxis(std::unique_ptr<InputButton> &&negative, std::unique_ptr<InputButton> &&positive) :
	...
	negative->OnButton().connect([this](InputAction action, bitmask::bitmask<InputMod> mods) {
		onAxis(GetAmount());
	});
	positive->OnButton().connect([this](InputAction action, bitmask::bitmask<InputMod> mods) {
		onAxis(GetAmount());
	});
}
...
void ButtonInputAxis::SetNegative(std::unique_ptr<InputButton> &&negative) {
	...
	this->negative->OnButton().connect([this](InputAction action, bitmask::bitmask<InputMod> mods) {
		onAxis(GetAmount());
	});
}
...
void ButtonInputAxis::SetPositive(std::unique_ptr<InputButton> &&positive) {
	...
	this->positive->OnButton().connect([this](InputAction action, bitmask::bitmask<InputMod> mods) {
		onAxis(GetAmount());
	});
}
